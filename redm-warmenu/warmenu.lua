...
local menus = { }
local keys = { up = `INPUT_FRONTEND_UP`, down = `INPUT_FRONTEND_DOWN`, left = `INPUT_FRONTEND_LEFT`, right = `INPUT_FRONTEND_RIGHT`, select = `INPUT_FRONTEND_RDOWN`, back = `INPUT_FRONTEND_RRIGHT`, fast = `INPUT_SPRINT`}
...
local function drawText(text, x, y, font, color, scale, center, shadow, alignRight)
	...
	if shadow then
		...
		--Citizen.InvokeNative(0xF66090013DE648D5, 2, 2, 0, 0, 0)
	end
	...
	Citizen.InvokeNative(0x50C803A4CD5932C5,color.r, color.g, color.b, color.a) 		--SetTextColour(color.r, color.g, color.b, color.a) now not needed anymore?
...
local function drawTitle()
	if menus[currentMenu] then
		...
		x = menus[currentMenu].x
		drawText(menus[currentMenu].title, x, y - titleHeight / 2 + titleYOffset, menus[currentMenu].titleFont, menus[currentMenu].titleColor, titleScale, false)
	end
end
	...
	if isMenuVisible(currentMenu) then
		if menus[currentMenu].aboutToBeClosed then
			...
			if IsControlJustPressed(1, keys.down) then
				PlaySoundFrontend("NAV_DOWN", "Ledger_Sounds", 1)
				if menus[currentMenu].currentOption < optionCount then
					menus[currentMenu].currentOption = menus[currentMenu].currentOption + 1
				else
					menus[currentMenu].currentOption = 1
				end
			elseif IsControlJustPressed(1, keys.up) then
				PlaySoundFrontend("NAV_UP", "Ledger_Sounds", 1)
			...
			elseif IsControlJustPressed(1, keys.select) then
				currentKey = keys.select
			elseif IsControlJustPressed(1, keys.back) then
				if menus[menus[currentMenu].previousMenu] then
					PlaySoundFrontend("BACK", "HUD_SHOP_SOUNDSET", 1)
...
local function IsControlJustPressed(c, i)
	local v = Citizen.InvokeNative(0x580417101DDB492F, c, i, Citizen.ReturnResultAnyway())
	return v
end

local function IsControlPressed(c, i)
	return Citizen.InvokeNative(0xF3A21BCD95725A4A, c, i, Citizen.ReturnResultAnyway())
end
